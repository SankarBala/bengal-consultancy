    ...
    <div class="p-3">
        <form action="{{ route('admin.client.update', $client) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                ...
                <div class="col-75">
                    <input type="text" id="name" name="name" placeholder="Client name.." value="{{ $client->name }}">
                    @error('name')
            ...
            <div class="row">
                ...
                <div class="col-35">
                    <input type="text" id="nid" name="nid" placeholder="Client's NID no.." value="{{ $client->nid }}">
                    @error('nid')
                        <p class="text-danger">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="row">
                ...
                <div class="col-35">
                    <select class="" name="preferred_country">
                        <option value="0" {{ $client->preferred_country == 0 ? 'selected' : '' }}>Select one</option>
                        @foreach ($countries as $country)
                            <option value="{{ $country->id }}"
                                {{ $client->preferred_country == $country->id ? 'selected' : '' }}>
                                {{ $country->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="row">
                ...
                <div class="col-35">
                    <textarea class="" rows="5" name="address"> {{ $client->address }} </textarea>
                </div>

                <div class="col-25">
                    <label class="w-100 text-center-md" for="status">Client's Status</label>
                </div>
                <div class="col-35 text-right">
                    <select name="status" id="status">
                        @foreach ($clientStatus as $status)
                            <option {{ $status->id == $client->status ? 'selected' : '' }} value="{{ $status->id }}">
                                {{ $status->title }}</option>
                        @endforeach
                        <select>
                            <button class="btn btn-primary my-3 px-4" type="submit">Update</button>
                </div>
            </div>
